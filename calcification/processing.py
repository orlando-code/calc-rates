...
# custom
from calcification import file_ops, locations, taxonomy, units, config
import cbsyst as cb
...
def preprocess_df(df: pd.DataFrame, selection_dict: dict={'include': 'yes'}) -> pd.DataFrame:
    ...
    # general processing
    df.rename(columns=file_ops.read_yaml(config.resources_dir / "mapping.yaml")['sheet_column_map'], inplace=True)    # rename columns to agree with cbsyst output    
...
def process_raw_data(df: pd.DataFrame, require_results: bool=True, selection_dict: dict={'include': 'yes'}, ph_conversion: bool=True) -> pd.DataFrame:
    ...
    ### units
    df['irr'] = df.apply(lambda row: units.irradiance_conversion(row['ipar'], 'PAR') if pd.notna(row['ipar']) else row['irr'], axis=1)  # convert integrated irradiance to irradiance
    
    if ph_conversion:   
        df['hplus'] = df.apply(lambda row: units.ph_to_hplus(row['phtot']) if pd.notna(row['phtot']) else None, axis=1) # convert pH to H+ concentration in μmol/kg seawater
    ...
    df[['st_calcification', 'st_calcification_sd', 'st_calcification_unit']] = df.apply(
        lambda x: pd.Series(units.rate_conversion(x['calcification'], x['calcification_sd'], x['st_calcification_unit'])) if pd.notna(x['calcification']) and pd.notna(x['st_calcification_unit']) else pd.Series(['', '', '']), 
        axis=1)
