...
# R Integration: Unified RContextManager, clean MetaforAdapter
# Data Access: DataLoader, DataValidator, DataExporter
# Plotting: PlotlyRenderer, consistent themes
...
# class StreamlitMetaforAdapter(metafor.MetaforModel):
#     """
...
#         if verbose:
#             print("MetaforModel initialized")
...
#             if "beta" in self.r_model:
#                 self.coefficients = np.array(list(self.r_model["beta"]))
#                 self.model_dict["beta"] = self.coefficients

#                 # Extract coefficient names
#                 self.coefficient_names = self._extract_coefficient_names_from_model()
...
#                     "})"
#                 )
#                 if len(labels) == len(self.coefficients.flatten()):
#                     if self.verbose:
#                         print(f"Using call attribute names: {labels}")
#                     return list(labels)

#             # Method 2: Try to get from X matrix column names (if available)
#             if "X" in self.r_model:
#                 X = self.r_model["X"]
#                 if hasattr(X, "colnames") and X.colnames is not None:
#                     names = list(X.colnames)
#                     if len(names) == len(self.coefficients.flatten()):
#                         if self.verbose:
#                             print(f"Using X matrix column names: {names}")
#                         return names

#             # Method 3: Try to extract from R context if safe
#             coef_names = self._try_extract_from_r_context()
#             if coef_names and len(coef_names) == len(self.coefficients.flatten()):
#                 if self.verbose:
#                     print(f"Using R context names: {coef_names}")
#                 return coef_names

#             # Method 4: Parse from formula (improved logic)
#             coef_names = self._parse_coefficient_names_from_formula()
#             n_coef = len(self.coefficients.flatten())

#             if len(coef_names) == n_coef:
#                 if self.verbose:
#                     print(f"Using formula-parsed names: {coef_names}")
#                 return coef_names
...
class DataLoader:
    ...
    def load_data(_self) -> pd.DataFrame:
        ...
        if Path(_self.data_path).exists():
            try:
                df = pd.read_csv(_self.data_path)
                st.info(f"✅ Loaded data from: {_self.data_path}")
                ...
                raise RuntimeError(f"Error loading {_self.data_path}: {e}")
