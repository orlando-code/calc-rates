...
```
```{r}
run_meta_analysis <- function(data, mods_formula, threshold = None, parallel_opts = list(type = "multicore", ncpus = 8)) {
  ...
  # if threshold in argument is None, compute
  if (is.null(threshold)) {
    threshold <- threshold(data, mods_formula)
  }
  # threshold <- 
...
phtot_results <- run_meta_analysis(
  data = phtot_hedges,
  mods_formula = ~ delta_pH + phtot + factor(genus) - 1 # -ve 1 removes the model intercept
)
...
svg(file = 'output/phtot_overall_bubble_with_id_nested.svg', width = 8, height = 8)
