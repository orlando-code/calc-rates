...
adult.data.hedges <- read.csv(file = "meta_2022/adult_raw_data(in).csv") %>%
  filter(Family != "") %>%  # remove rows where Family field is empty
  filter(!is.na(dpHt)) %>%  # remove rows with missing dpHt values
  mutate(id = row.names(.)) %>% # create an id field
  mutate(correction = (1 - (3/(4*(n*2)-9)))) %>%  # calculate small-sample bias correction factor
  mutate(corrected_hedges = Hedges.G * correction) %>%  # apply bias correction
  mutate(corrected_variance = Variance * (correction^2))



# multiple corrected_hedges by SQRT(n)

adult.data.hedges <- adult.data.hedges %>%
  mutate(n_corrected_hedges = corrected_hedges * 1/sqrt(n))

```

```{r}
# print the first 5 values of corrected_hedges and n_corrected_hedges columns
head(adult.data.hedges$corrected_hedges, 5)
head(adult.data.hedges$n_corrected_hedges, 5)
...
model.full.adult.hedges.id.nested <- rma.mv(yi = n_corrected_hedges, 
...
model.full.adult.hedges.cooks <- cooks.distance.rma.mv(model.full.adult.hedges.id.nested, progbar=TRUE,
        reestimate=TRUE, parallel="multicore", ncpus=1, cl=NULL)
...
adult.outliers <- model.full.adult.hedges.cooks %>% 
  as_tibble() %>% 
  mutate(Study = seq(1:nrow(adult.data.hedges))) %>%
  filter(value > 0.31)  # hardcoded threshold
...
model.full.adult.hedges.id.nested <- rma.mv(yi = n_corrected_hedges, 
              ...
              method = "REML", 
              # mods = ~ dpHt + Temperature + Irradiance..umol.photons.m.2.s.1. + Duration..days. + factor(Family) - 1,
              mods = ~ dpHt + Temperature + factor(Family) - 1,
              test="z",
              random = list(~ 1| (id/Study)))
...
svg(file = 'output/ben_adult_overall_bubble_with_id_n_corrected_no_extras.svg', width = 8, height = 8) 
...
```{r}
# filter adult data for climate == "tropical" and re-run
adult.data.hedges.tropical <- adult.data.hedges %>% filter(Climate == "Tropical")


model.tropical.adult.hedges.id.nested <- rma.mv(yi = corrected_hedges, 
              V = corrected_variance, 
              data = adult.data.hedges.tropical, 
              method = "REML",
              mods = ~ dpHt + Temperature + Irradiance..umol.photons.m.2.s.1. + Duration..days. + factor(Family) - 1,
              test="z",
              random = list(~ 1| (id/Study)))
...
model.tropical.adult.hedges.cooks <- cooks.distance.rma.mv(model.tropical.adult.hedges.id.nested, progbar=TRUE,
        reestimate=TRUE, parallel="multicore", ncpus=1, cl=NULL)

plot(model.tropical.adult.hedges.cooks, type="o", pch=19, xlab="Study", ylab="Cook's Distance", xaxt="n")
axis(side=1, at=seq_along(model.tropical.adult.hedges.cooks), labels=as.numeric(names(model.tropical.adult.hedges.cooks)))

#Those studies with a Cook's distance greater than the threshold are removed. Threshold is ascertained by a conservative cut-off threshold of 2√((k+1)/(n - k - 1)). 

adult.outliers.tropical <- model.tropical.adult.hedges.cooks %>% 
  as_tibble() %>% 
  mutate(Study = seq(1:nrow(adult.data.hedges.tropical))) %>%
  filter(value > 0.31)  # hardcoded threshold

adult.data.hedges.tropical <- adult.data.hedges.tropical[-c(adult.outliers.tropical$Study),]

rm(adult.outliers.tropical)
```
...
svg(file = 'output/ben_tropical_adult_overall_bubble_with_id_nested.svg', width = 8, height = 8) 
regplot.rma(model.tropical.adult.hedges.id.nested, mod = "dpHt", refline = 0, xlim = c(-1,0.25), ylim = c(-15,15), predlim = c(-1,0.25),
            ylab = "Hedges' g", xlab = expression(paste("\U0394", "pH")[T]))
dev.off() 
